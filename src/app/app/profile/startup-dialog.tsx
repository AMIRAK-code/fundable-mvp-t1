...
import { X, Camera, Code2, Link2, Globe, ExternalLink, Loader2 } from 'lucide-react'
...
export default function StartupDialog({ open, onOpenChange, startup }: Props) {
  ...
  // Returning null on close unmounts StartupForm, so useActionState resets
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h3 id="startup-dialog-title" className="font-semibold text-lg">
        ...
        <StartupForm startup={startup} onClose={() => onOpenChange(false)} />
      </div>
    </div>
  )
}

function StartupForm({ startup, onClose }: { startup: Startup | null; onClose: () => void }) {
  ...
  // Revoke object URL on unmount (form remounts each open)
  useEffect(
    () => () => {
      if (previewRef.current) URL.revokeObjectURL(previewRef.current)
    },
    ...
    <form action={action} className="space-y-4">
      ...
      <div className="flex flex-col items-center gap-2">
        <button
          ...
          className="press-subtle relative w-full h-32 rounded-xl border-2 border-dashed border-white/20 hover:border-[var(--brand-primary)] transition-colors overflow-hidden bg-white/5 flex items-center justify-center"
        >
      ...
      <div className="space-y-1.5">
        ...
        <input name="name" required maxLength={80} defaultValue={startup?.name} placeholder="Acme AI"
      ...
      <div className="space-y-1.5">
        ...
        <input name="pitch" required maxLength={200} defaultValue={startup?.pitch} placeholder="We automate X for Y companies"
          className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]" />
      </div>

      <div className="space-y-1.5">
        <label className="text-sm text-muted-foreground">Industry</label>
        <input name="industry" maxLength={80} defaultValue={startup?.industry ?? ''} placeholder="B2B SaaS, FinTech, HealthTech…"
          className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]" />
      </div>
